...
type TProductsBoardInDiscountProps = {
	page: number;
	setPage: React.Dispatch<React.SetStateAction<number>>;
	pageSize?: number;
	openViewListSchedulesOnClick?: (product: TProductSumary) => void;
	className?: string;
};

export const ProductsBoardInDiscount = ({
	page,
	setPage,
	pageSize = 10,
	openViewListSchedulesOnClick = (): void => {},
	className,
}: TProductsBoardInDiscountProps): JSX.Element => {
	...
	const columnTable: TableColumnsType<TProductSumary> = [
		...
		},
		{
			...
			render: (_, record: TProductSumary) => (
				<button
					type="button"
					onClick={() => openViewListSchedulesOnClick(record)}
