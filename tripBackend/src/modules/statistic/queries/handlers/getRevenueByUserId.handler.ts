...
import { GetRevenueByUserIdQuery } from '../implements';

@QueryHandler(GetRevenueByUserIdQuery)
export class GetRevenueByUserIdHandler implements IQueryHandler<GetRevenueByUserIdQuery> {
	...
	async execute(
		...
		const revenueData = await this.statisticRepository.getRevenueByUserId(
			myInformation.id,
		...
		for (const item of revenueData) {
			...
			groupMap[key] = (groupMap[key] || 0) + Number(item._sum.amount);
		}
